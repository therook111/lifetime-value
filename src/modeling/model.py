...
class LinearModel(nn.Module):
    def __init__(self, df: pd.core.frame.DataFrame, output_units=3):
        ...
        self.model = nn.Linear(in_features=len(feature_dict), out_features=output_units)

    def forward(self, inputs):
        numeric_input = inputs[self.numeric_features[0]]

        categorical_input = [inputs[key] for key in self.categorical_features]

        final_input = torch.cat([numeric_input] + categorical_input, dim=1)
        return self.model(final_input)
